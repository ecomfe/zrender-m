...
define('zrender/tool/color',['require','../tool/util'],function(require) {
    ...
    function getNameColor(name) {
        if (name instanceof Array || name.indexOf('rgb') != -1 || name.indexOf('hs') != -1) {
            ...
            _nameColors[name] = toColor([data[0], data[1], data[2], data[3]], 'rgba');
        }
 ...
 */
define(
    'zrender/zrender',['require','./tool/util','./tool/log','./tool/guid','./Handler','./Painter','./Storage','./animation/Animation','./tool/env'],function(require) {
        var util = require('./tool/util');
        var log = require('./tool/log');
        var guid = require('./tool/guid');
